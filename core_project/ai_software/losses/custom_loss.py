...
class SobelEdgeLoss(nn.Module):
    """
    Hàm mất mát cấu trúc biên cạnh Sobel (Sobel Edge Loss).
    Sử dụng hai bộ lọc đạo hàm không gian Sobel 3x3 cố định theo trục X và Y
    để trích xuất gradient và tính toán sai số biên cạnh giữa ảnh dự đoán và Ground Truth.
    """
    ...
    def __init__(self, eps: float = 1e-6) -> None:
        ...
        # Đóng gói thành Conv2d cố định không cập nhật đạo hàm
    ...
    def forward(self, pred: torch.Tensor, target: torch.Tensor) -> torch.Tensor:
        """
        Tính sai số L1 giữa bản đồ biên độ gradient Sobel của ảnh dự đoán và ảnh đích.

        Args:
            pred: Tensor ảnh dự đoán [Batch, 1, H, W]
            target: Tensor ảnh Ground Truth [Batch, 1, H, W]

        Returns:
            Scalar tensor chứa giá trị Sobel Edge Loss.
        """
...
class SRCNN_CompositeLoss(nn.Module):
    """
    Hàm mất mát kết hợp theo yêu cầu mục 1.1 trong Request1.txt:
        Loss = MSE + 0.1 * Sobel_Edge_Loss
    """

    def __init__(self, sobel_weight: float = 0.1) -> None:
        super().__init__()
    ...
    def forward(
        ...
        """
        Forward pass tính tổng loss và trả về chi tiết từng thành phần.

        Args:
            pred: Ảnh dự đoán [Batch, 1, H, W]
            target: Ảnh Ground Truth [Batch, 1, H, W]

        Returns:
            total_loss: Tensor tổng loss có thể gọi .backward()
            loss_dict: Dictionary chứa giá trị float của MSE và Sobel loss để logging
        """
