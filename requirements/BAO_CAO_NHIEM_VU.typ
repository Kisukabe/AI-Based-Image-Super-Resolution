...
#align(center)[
  #table(
    ...
    [D1.1], [Checkpoint PyTorch Compact SRCNN Float32 (1.649 tham số)], [`core_project/ai_software/checkpoints/compact_srcnn_float32.pth`], [#badge-pass([ĐÃ HOÀN THÀNH])], [D1.2], [Script tạo tập dữ liệu suy thoái vật lý y tế], [`generate_degraded_dataset.py`], [#badge-pass([ĐÃ HOÀN THÀNH])], [D1.3], [Script đánh giá mô hình baseline trên tập test], [`core_project/ai_software/evaluation/evaluate_pytorch_baseline.py`], [#badge-pass([ĐÃ HOÀN THÀNH])], [D1.4], [File ảnh đồ thị hội tụ hàm Loss (300 DPI, 4200x1500)], [`core_project/ai_software/training/loss_convergence_dpi300.png`], [#badge-pass([ĐÃ HOÀN THÀNH])], [D1.5], [Bảng số liệu đối chứng PSNR/SSIM (Excel 4 Sheet, CSV, JSON)], [`core_project/benchmarks_reports/deliverables_export/baseline_psnr_ssim_comparison.xlsx`], [#badge-pass([ĐÃ HOÀN THÀNH])]
  )
]
...
#align(center)[
  #table(
    ...
    [*Xilinx PYNQ-Z2 (Zynq-7020 FPGA)*], [S7.0 / S0.7 (Fixed-Point)], [*444.190 ± 0.52*], [*1.438*], [*2.25*], [*1.56*], [NVIDIA Tesla T4 (Datacenter GPU)], [Float32 (CUDA 12.2)], [18.530 ± 0.15], [35.000], [53.97], [1.54], [Apple M1 CPU (arm64)], [Float32], [19.347 ± 0.48], [8.200], [51.69], [6.30], [Apple M1 GPU (Metal MPS)], [Float32], [11.890 ± 0.35], [9.500], [84.10], [8.85], [Intel/AMD x86_64 CPU (AVX2/MKL)], [Float32], [45.200 ± 1.10], [45.000], [22.12], [0.49]
  )
]
...
#align(center)[
  #table(
    ...
    stroke: (x, y) => if y == 0 { (bottom: 1.5pt + brand-accent) } else { 0.5pt + border-color },
    table.header([*Mã*], [*Sản phẩm bàn giao*], [*File lưu trữ*], [*Trạng thái*]),
    [D2.1], [Notebook Kaggle Benchmark Compact SRCNN RTL (13 cell, 3 scale 2x/3x/4x)], [`legacy_experiments/notebooks_kaggle/hardware/kaggle_compact_srcnn_hardware_benchmark.ipynb`], [#badge-pass([ĐÃ HOÀN THÀNH])], [D2.2], [Script phân tích JSON và bộ biểu đồ Histogram (300 DPI)], [`analyze_kaggle_benchmark.py`, `figures_dpi300/hist_*.png`], [#badge-pass([ĐÃ HOÀN THÀNH])], [D2.3], [Script kiểm thử Bit-Exact DV Scoreboard (4/4 kịch bản ALL_PASS)], [`core_project/hardware_fpga/dv_verification/dv_scoreboard_check.py`], [#badge-pass([ĐÃ HOÀN THÀNH])], [D2.4], [Bảng Excel thống kê 2.200 ảnh và đo đạc thực nghiệm đa nền tảng (6 Sheet)], [`hardware_benchmark_statistics.xlsx`], [#badge-pass([ĐÃ HOÀN THÀNH])], [D2.5], [Thư mục 29 hình ảnh độ phân giải cao 300 DPI (Histogram, ROI, Heatmap)], [`figures_dpi300/`], [#badge-pass([ĐÃ HOÀN THÀNH])], [D2.6], [Báo cáo so sánh 8 mô hình siêu phân giải song ngữ VN/EN (2 bản PDF)], [`performance_comparison_report.pdf`, `performance_comparison_report_en.pdf`], [#badge-pass([ĐÃ HOÀN THÀNH])], [D2.7], [Hình Fig. 7 Bài báo IEEE: Overlap-Tiling Boundary Ablation (300 DPI)], [`figures_dpi300/fig7_boundary_ablation.png`], [#badge-pass([ĐÃ HOÀN THÀNH])], [D2.8], [Hình Fig. 8 Bài báo IEEE: Ma trận so sánh định tính 2 hàng x 7 mô hình (300 DPI)], [`figures_dpi300/fig8_visual_comparison.png`], [#badge-pass([ĐÃ HOÀN THÀNH])]
  )
]
